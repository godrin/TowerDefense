...
public abstract class MovingShot extends PositionedDisplayEffect implements
	...
	public void move(float time) {
		...
		if (curTime > allTime) {
			pos = target;
			Unit unit = level.getUnitAt(target, Unit.class);
			if (unit != null && unit instanceof ShotTarget) {
				ShotTarget t = (ShotTarget) unit;
	...
	protected void drawBurn(IRenderer renderer) {
		...
		s.init();
		renderer.render(s, Position.NULL, 1, 0, GL20.GL_TRIANGLES);
